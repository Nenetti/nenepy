...
from nenepy.torch.utils.tensorboard.tensorboard import TensorBoard, Type
...
class TensorBoardWriter(TensorBoard):
    ...
    def add_scalars(self, namespace, graph_name, scalar_dict, step):
        ...
        """
        self._writer.add_scalars(main_tag=self._to_scalars_tag(namespace, graph_name), tag_scalar_dict=scalar_dict, global_step=step)
    ...
    def add_images(self, namespace, image_dict, step):
        ...
        for name, image in image_dict.items():
            self.add_image(namespace=self._to_image_tag(namespace, name), image=image, step=step)
