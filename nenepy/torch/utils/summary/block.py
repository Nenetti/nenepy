...
class Values:
    ...
    def recursive(cls, value):
        if isinstance(value, (tuple, list, set)):
            if cls.contains_iterable(value):
                pass
            else:
                return [cls.recursive(value) for value in value]

        elif isinstance(value, dict):
            if cls.contains_iterable(value.values()):
                pass
            else:
                return [cls.recursive(value) for value in value]
...
class Value:
    ...
    def __init__(self, type, text, length, is_tensor):
        pass
